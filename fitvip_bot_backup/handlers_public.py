...
def setup_public_handlers(dp: Dispatcher, bot):
    ...
    def _first_existing_img(path_wo_ext: str) -> FSInputFile:
    ...
    async def _get_support_photo() -> Union[str, FSInputFile]:
    ...
    async def process_subscription(callback: types.CallbackQuery, state: FSMContext):
        ...
        # Показываем информацию о стоимости курса
        pricing_text = (
            "💰 Стоимость курса\n\n"
        ...
        # Кнопка для перехода к вводу суммы
    ...
    # Обработчик кнопки "Ввести сумму"
    ...
    async def enter_amount(callback: types.CallbackQuery, state: FSMContext):
        ...
        amount_text = (
            ...
            "🔹 Базовая стоимость: 1500 рублей\n"
            "🔹 + 100 рублей за каждый день персонального ведения (макс. 15 дней)\n\n"
            "💡 Примеры:\n"
            "• 1500 руб. — только курс\n"
            "• 2000 руб. — курс + 5 дней ведения\n"
            "• 3000 руб. — курс + 15 дней ведения\n\n"
    ...
    async def process_amount_input(message: types.Message, state: FSMContext):
        try:
            ...
            if amount > 3000:
                amount = 3000
                await message.answer("💡 Максимальная сумма 3000 рублей. Используем эту сумму.")
            
            # Рассчитываем тариф
            from tariffs import calculate_tariff
            tariff_info = calculate_tariff(amount)
            
            if not tariff_info:
                await message.answer("❌ Ошибка расчета тарифа. Попробуйте ещё раз.")
                return
            ...
            payment_text = (
                ...
                f"📅 {tariff_info['duration']}\n\n"
                "Выберите способ оплаты:"
            )
            
            # Получаем доступные способы оплаты из БД
            from db import get_payment_methods
            methods_from_db = await get_payment_methods()
            
            from keyboards import payment_methods
            kb = payment_methods('custom', methods_from_db)
            
            # Сохраняем сумму для дальнейшей обработки
            ...
            await message.answer(
                "❌ Пожалуйста, введите число от 1500 до 3000:",
                reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                    [types.InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_payment")]
                ])
            )
    ...
    async def process_about_channel(callback: types.CallbackQuery, state: FSMContext):
        ...
        agreement_text = (
            ...
            "1.2. Сервис – Telegram-бот @fit_vip_bot, принадлежащий Исполнителю и предназначенный для продажи Товара.\n"
            ...
            "2.2. Сервис @fit_vip_bot является независимой площадкой и не является аффилированным лицом Telegram FZ-LLC.\n\n"
            "3. Порядок оплаты\n"
            ...
            "Полная версия: https://telegra.ph/User-Agreement-Fit-VIP-Bot"
        )
